...
class Controller:
    ...
    def __init__(self, x, y):
        ...
        # self.__minSpannTree = self.minimumSpanningTree()
    ...
    def minimumSpanningTree(self):
        ...
        minSpanningTree = self.__environment.getEdges().copy()
        visited = []
        queue = []
        ...
        while len(queue) > 0:
            ...
            del queue[0]
            index = 0
            listCurrent = minSpanningTree[current].copy()
            lenCurrent = len(listCurrent)
            while index < lenCurrent:

                if listCurrent[index] not in visited:
                    visited.append(listCurrent[index])
                    queue.append(listCurrent[index])
                    index = index + 1;

                else:

                    i = 0
                    while i < len(minSpanningTree[current]):
                        if listCurrent[index] == minSpanningTree[current][i]:
                            del minSpanningTree[current][i]
                        else:
                            i += 1
                    index += 1
        return minSpanningTree

    def moveOptimizedDSF(self, minSpannTree):

        # We mark the current position as being visited
        self.__drone.visited.append((self.__drone.x, self.__drone.y))
        currentNode = (self.__drone.x, self.__drone.y)
        # The order will be UP - LEFT - DOWN - RIGHT
        for neighbor in minSpannTree[currentNode]:
            new_X = neighbor[0]
            new_Y = neighbor[1]
            # We check if we have not visited this square before
            if (new_X, new_Y) not in self.__drone.visited:
    ...
    def minimumSpanningTree2(self, screen):

        queue = []
        x = self.__drone.x
        y = self.__drone.y

        # We add the current node to the queue
        queue.append((x, y))

        # We mark the current node as being visited
        self.__drone.visited.append((x, y))

        while len(queue) > 0:
            # We take the first element from the queue and after that we delete it
            current = queue[0]
            del queue[0]
            index = 0
            listCurrent = self.__environment.getEdges()[current].copy()
            lenCurrent = len(listCurrent)
            while index < lenCurrent:

                # We set the new coordinates of the drone
                self.__drone.x = listCurrent[index][0]
                self.__drone.y = listCurrent[index][1]

                # We check if the neighbor of the current node was visited
                if listCurrent[index] not in self.__drone.visited:

                    # We mark it as being visited
                    self.__drone.visited.append(listCurrent[index])

                    # We update the map
                    self.markDetectedWalls()
                    screen.blit(self.getDMapImage(), DMAP_BLIT)
                    time.sleep(SLEEP)
                    pygame.display.flip()

                    # We add the neighbor to the queue
                    queue.append(listCurrent[index])
                    index = index + 1;

                else:

                    # If the neighbor was already visited, we remove the edge between him and the current node
                    i = 0
                    while i < len(self.__environment.getEdges()[current]):
                        if listCurrent[index] == self.__environment.getEdges()[current][i]:
                            del self.__environment.getEdges()[current][i]
                        else:
                            i += 1
                    index += 1

        # When the number of visited squares is equal to the number of nodes we mark the stop condition
        if len(self.__drone.visited) == len(self.__environment.getNodes()):
            self.__drone.x = None
            self.__drone.y = None